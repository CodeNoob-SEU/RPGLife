...
export type QuestDraft = { name: string; gold: string; exp: string; icon: string };
...
export function QuestFormModal({
  ...
  initial: QuestDraft;
  onCancel: () => void;
  onSave: (v: { name: string; gold: number; exp: number; icon: string }) => void;
}) {
  ...
  useEffect(() => {
    if (visible) { setName(initial.name); setGold(initial.gold); setExp(initial.exp); setIcon(initial.icon); }
