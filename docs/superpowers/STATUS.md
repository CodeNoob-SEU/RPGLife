...
- ✅ 分类/标签 + 筛选、CSV 流水导出、禁忌任务(anti-habits) — 已交付
- ☐ 剩余：音效(expo-audio，触感已接)、外观随等级解锁/皮肤主题商店（多调色板切换需 theme context 重构）、打卡留证（照片/日志）、灵活排程(每周N次/隔日)、i18n 英文包、可分享成就卡(view-shot)、CRT 扫描线主题。
- 新增持久化字段沿用 `CURRENT_VERSION` +1 + `migrate` 深填策略（当前已到 **v9**）。
