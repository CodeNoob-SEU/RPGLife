...
> 进度标记：☐ 待办 / ▶ 进行中 / ✅ 完成。
>
> **进度（2026-06-01）：阶段 0–4 的 OP-0a…OP-17 全部 ✅ 完成并验证（见 git log 18 个 commit + STATUS「商业化迭代」）。**
> **未做（阶段 5 拉伸 + OP-18）：OP-18 本地通知、OP-19 分类/标签、OP-20 主题皮肤、OP-21 CRT、OP-22 可分享成就卡、OP-23 灵活排程、OP-24 i18n。** 沿用 `CURRENT_VERSION` +1 + migrate 深填策略（已至 v7）。
