...
> 进度标记：☐ 待办 / ▶ 进行中 / ✅ 完成。
>
> **进度（2026-06-01）：OP-0a…OP-19 全部 ✅ 完成并验证（git log 21 个 commit + 代码审查通过 + STATUS「商业化迭代」）。**含阶段 0-4 全部 + OP-18 本地通知 + OP-19 分类/标签 + 经济下限加固。
> **未做（阶段 5 拉伸）：OP-20 主题皮肤(需 theme context 重构)、OP-21 CRT、OP-22 可分享成就卡、OP-23 灵活排程、OP-24 i18n、禁忌任务、音效、打卡留证。** 沿用 `CURRENT_VERSION` +1 + migrate 深填策略（已至 **v8**）。
