...
> 进度标记：☐ 待办 / ▶ 进行中 / ✅ 完成。
>
> **进度（2026-06-01）：阶段 0-4 全部 + 本地通知 + 分类/标签 + CSV 导出 + 禁忌任务 + 经济加固，全部 ✅ 完成并验证（git log 24 个 commit + 代码审查通过 + STATUS「商业化迭代」）。**
> **未做（拉伸）：主题皮肤(需 theme context 重构)、CRT 扫描线、可分享成就卡、灵活排程(改结算引擎)、i18n、音效、打卡留证。** 沿用 `CURRENT_VERSION` +1 + migrate 深填（已至 **v9**）。
